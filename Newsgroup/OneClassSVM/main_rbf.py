...
nulist = np.arange(1e-5, 0.4, 0.05)[:3]
Mlist = np.arange(14,32,2)
gammalist = np.logspace(-4, 4, 50)
F1score = np.zeros((len(Mlist), len(nulist),len(gammalist)))
done = np.zeros((len(Mlist), len(nulist),len(gammalist)))
if False:
	fpres = open('results_rbf.txt','r')
	ndone = 0
	while True:
		ln = fpres.readline() # star line
		if len(ln) == 0:
			break
		ln1 = fpres.readline() # group 1
		ln2 = fpres.readline() # group 2
		ln3 = fpres.readline() # res
		res = float(re.findall(r"F1-score = ([0-9\.]*)\n", ln3)[0])
		M = int(re.findall(r"M = ([0-9]*),", ln3)[0])
		nu = float(re.findall(r"nu = ([0-9\.]*),", ln3)[0])
		gamma = float(re.findall(r"gamma = ([0-9\.]*),", ln3)[0])
		m1 = np.where(Mlist==M)[0][0]
		n1 = np.argmin(np.fabs(nulist-nu))
		g1 = np.argmin(np.fabs(gammalist-gamma))
		print(n1,g1,m1, nulist[n1],gammalist[g1],res, M,nu,gamma)
		F1score[m1,n1,g1] = res
		done[m1,n1,g1] = 1
		ndone += 1
	fpres.close()
else:
...
for n1,nu in enumerate(nulist):
	for g1,gamma in enumerate(gammalist):
		...
		if np.sum(done[:,n1,g1])==len(Mlist):
			continue
		print (nu,gamma, (done[:,n1,g1]), len(Mlist))
		# train svm
		...
		for m1,M in enumerate(Mlist):
			...
			if n1==0 and g1==0:
				...
				cmdtxt = LDAPath + '/lda ' + str(seed) + ' est 0.1 ' + str(M) + ' ' + LDAPath + '/settings.txt ' + testfile + ' seeded dirlda' + str(M) 
